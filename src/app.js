...
function renderAll(button,state) {
  renderPatty();
  renderCheese();
  renderTomatoes();
  renderOnions();
...
function renderButtons(buttonName, buttonState) {
...
function renderIngredientsBoard(){
  for (var item in state) {
    ...
    for (var ingredient of items) {
      if (item == ingredient.innerHTML) {
        if (state[item]) ingredient.style.display = "inherit";
        else ingredient.style.display = "none";
      }
    }
  }
}
...
function renderPrice(){
  ...
  for (each in state) {
    if(state[each]==true){
      price+=ingredients[each];
    }
  }
  console.log(price);
  document.querySelector(".price-details").innerHTML="Total Price: "+price;
}